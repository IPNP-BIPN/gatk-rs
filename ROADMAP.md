...
| **gatk-rs** | 202 tools | 5 crates, **43 conformance suites, all oracle-backed**; 1 tool byte-identical, and the annotation archetype opened with 44 of 54 annotations measured |
...
- [~] the 54-annotation library. **Forty-four** are ported and oracle-backed: the counting family
      ...
      allele-specific strand-bias pair (`AS_FS`, `AS_SOR`, over the `AS_SB_TABLE` they share),
      together with the
...
- [ ] the remaining 10. **The claim that most of them wait on jmath does not survive a grep**: 10
