...
- [x] **C.3 the file plumbing**, a path where a port takes a `&str` today. `condense_depth_evidence::read`
      ...
      the file (`count-variants`). One hundred tools have it today, and the layer is one shape
      rather than one hundred: a BAM output carries the index and the digest its arguments ask for
...
- [x] **C.5 the covering arrays run against the port binary**, which is the point of the milestone
      and what unblocks V.5. One hundred tools are measured on both sides on every CI run, all of
      ...
      metrics file's `Started on` line is the wall clock and is compared without it
