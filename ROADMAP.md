...
- [x] **C.3 the file plumbing**, a path where a port takes a `&str` today. `condense_depth_evidence::read`
      ...
      the file (`count-variants`). A hundred and fifteen tools have it today, and the layer is one
      shape rather than a hundred and fifteen: a BAM output carries the index and the digest its arguments ask for
...
- [x] **C.5 the covering arrays run against the port binary**, which is the point of the milestone
      and what unblocks V.5. A hundred and fifteen tools are measured on both sides on every CI run,
      ...
      had shown because each holds `--add-output-vcf-command-line` at false
