...
| **gatk-rs** | 202 tools | 5 crates, **33 conformance suites, all oracle-backed**; 1 tool byte-identical, and the annotation archetype opened with 15 of 54 annotations measured |
...
- [~] the 54-annotation library. **Fifteen** are ported and oracle-backed: the counting family
      ...
      (`BaseQRankSum`, `MQRankSum`, `ReadPosRankSum`, `ClippingRankSum`), together with the
      ...
      `VariantContextGetters`, and **`MannWhitneyU`** with commons-math3's `FastMath.exp`,
      `FastMath.log`, `Gamma`, `Erf` and `NormalDistribution` under it, all oracle-backed in
      htsjdk-rs)
- [ ] the remaining 39. **The claim that most of them wait on jmath does not survive a grep**: 10
