...
- [x] **C.5 the covering arrays run against the port binary**, which is the point of the milestone
      ...
      disabling RMSMappingQuality under the default groups is refused. GenotypeGVCFs, ReblockGVCF
      and GnarlyGenotyper followed on the same merge: a failed run still writes its partial
