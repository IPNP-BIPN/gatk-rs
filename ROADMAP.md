...
| **gatk-rs** | 202 tools | 5 crates, **42 conformance suites, all oracle-backed**; 1 tool byte-identical, and the annotation archetype opened with 42 of 54 annotations measured |
...
- [~] the 54-annotation library. **Forty-two** are ported and oracle-backed: the counting family
      ...
      `AS_ReadPosRankSum`, with the `Histogram` and `CompressedDataList` they travel as),
      together with the
...
- [ ] the remaining 12. **The claim that most of them wait on jmath does not survive a grep**: 10
