...
- [x] **C.3 the file plumbing**, a path where a port takes a `&str` today. `condense_depth_evidence::read`
      ...
      the file (`count-variants`). A hundred and sixteen tools have it today, and the layer is one
      shape rather than a hundred and sixteen: a BAM output carries the index and the digest its arguments ask for
...
- [x] **C.5 the covering arrays run against the port binary**, which is the point of the milestone
      and what unblocks V.5. A hundred and sixteen tools are measured on both sides on every CI run,
      ...
      writer. Once the arrays compared what a tool PRINTS (#1201), eighteen runners turned out to
      return nothing where `doWork` returns `SUCCESS` or `0`, or to return what the reference
      ...
      `addMetaDataLine` keeps a header line already declared under the same key and ID. The SV
      clustering pair, SVAnnotate and the concordance and Mutect utilities after them added that
      the `Exception thrown at` wrapper prints the record's DECODED genotypes once anything read
      them, that `--lenient` is what lets an undeclared FILTER through the writer, and that a
      metrics file's `Started on` line is the wall clock and is compared without it. The fifth
      lot's first nine found that GKL passes levels 1 and 2 through to ISA-L (htsjdk-rs decision
      0043, a 15 KB BGZF block being the first to tell them apart), that GZIPOutputStream writes
      an operating system byte of 255, that a collection set to `null` still counts as set and
      leaves two spaces in a recorded command line, and that a variant walker's `-L` resolves
      against the dictionary its Tribble index synthesizes when nothing else has one. The next six
      (VCFComparator, the three flow tools that read the flow matrix, FlowFeatureMapper and
      CRAMIssue8768Detector) found that a MultiVariantWalker names each input by its tag, so one
      fixture value has to stand for a whole set of tagged inputs (`$tagged`), that a collection
      initialised with elements prints one pair per element in the recorded command line, and that
      the shared `##GATKCommandLine` of every variant writer had lost its `Date=`, which no array
      had shown because each holds `--add-output-vcf-command-line` at false. CombineGVCFs, the
      first tool whose merge is ported rather than modelled, found that GATK writes a sample's
      stale genotype when a block left open at a contig's end meets that sample's next block,
      because `getGenotype` answers with a name's last occurrence (htsjdk-rs #237), and that
      Barclay records every selected annotation that declares an argument as a predecessor, so
      disabling RMSMappingQuality under the default groups is refused
