...
- [x] **C.3 the file plumbing**, a path where a port takes a `&str` today. `condense_depth_evidence::read`
      ...
      the file (`count-variants`). A hundred and nine tools have it today, and the layer is one
      shape rather than a hundred and nine: a BAM output carries the index and the digest its arguments ask for
...
- [x] **C.5 the covering arrays run against the port binary**, which is the point of the milestone
      and what unblocks V.5. A hundred and nine tools are measured on both sides on every CI run,
      ...
      against the dictionary its Tribble index synthesizes when nothing else has one
