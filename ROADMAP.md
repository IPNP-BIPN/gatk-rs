...
| **gatk-rs** | 202 tools | 5 crates, **41 conformance suites, all oracle-backed**; 1 tool byte-identical, and the annotation archetype opened with 39 of 54 annotations measured |
...
- [~] the 54-annotation library. **Thirty-nine** are ported and oracle-backed: the counting family
      ...
      (`ExcessHet`, `InbreedingCoeff`) with `MQ` and the tandem repeats (`STR`/`RU`/`RPA`),
      together with the
...
- [ ] the remaining 15. **The claim that most of them wait on jmath does not survive a grep**: 10
