...
| **gatk-rs** | 202 tools | 5 crates, **33 conformance suites, all oracle-backed**; 1 tool byte-identical, and the annotation archetype opened with 24 of 54 annotations measured |
...
- [~] the 54-annotation library. **Twenty-four** are ported and oracle-backed: the counting family
      ...
      (`AS_UNIQ_ALT_READ_COUNT`, `BQHIST`, `REF_BASES`) and the depth family (`AD`, `AF`, `DP`),
      together with the
...
- [ ] the remaining 30. **The claim that most of them wait on jmath does not survive a grep**: 10
