...
// The GATKRead accessors live in gatk-engine, because they are the engine's adapter and not the
// filters': `SAMRecordToGATKReadAdapter` is what every walker sees a read through. They are
// re-exported here so a filter still reads `read::is_unmapped(...)`.
pub use gatk_engine::read;
pub use gatk_engine::read::flags;
pub use gatk_engine::read::{
    MAPPING_QUALITY_UNAVAILABLE, NO_ALIGNMENT_REFERENCE_INDEX, NO_ALIGNMENT_START,
};
